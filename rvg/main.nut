...
class RVG extends GSController
{
    companies = null;
    towns = null;
    current_date = null;
...
function RVG::Run(){
    ...
    if (1 == this.actual_town_info_mode && system_time - this._past_system_time > 3) {
        this._past_system_time = system_time;
...
function RVG::Load(version, saved_data)
{
...
function RVG::UpdateCompanyList()
{
...
function RVG::CreateCompanyList()
{
    this.companies = [];
 ...
 */
function RVG::UpdateTownList(town_id)
{
    // Create list of cargos/industries near each town
    local near_town;
    if (::SettingsTable.randomization == Randomization.INDUSTRY_DESC ||
...
function RVG::DailyManageTownPopulation()
{
    foreach (town in this.towns) {
